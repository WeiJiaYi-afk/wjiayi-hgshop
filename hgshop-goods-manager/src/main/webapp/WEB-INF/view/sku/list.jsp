...
<div>
 ...
 <label>输入名称：</label> <input type="text" id="queryName" value="${skuVo.key}" >	
...
<div>
	<table class="table">
		...
		<c:forEach items="${pageInfo.list}" var="sku">
			...
			<td>
				<button type="button" class="btn btn-primary" onclick="toModify(${sku.id})">修改</button>
...
<script>
	...
	*/
	function query(page){
		$("#workcontent").load("/sku/list?page="+page + "&key=" + $("#queryName").val());
	}
	...
	function del(id){
		...
		delIds.push(id)
		$.post('/sku/delBatch',{ids:delIds},function(data){
			if(data=="success"){
				//删除成功
				alert("删除成功")
				//刷新页面
